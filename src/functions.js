...
const getNeightbors = (board, row, column) => {
  const neighbors = [];
...
const safeNeighborhood = (board, row, column) => {
  ...
  return getNeightbors(board, row, column).reduce(safes, true);
};
...
const openField = (board, row, column) => {
  ...
  if (!field.opened) {
    ...
    if (field.mined) {
      ...
      getNeightbors(board, row, column).forEach(n => {
        openField(board, n.row, n.column);
      });
    } else {
      const neighbors = getNeightbors(board, row, column);
